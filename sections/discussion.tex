...
This convergence does not yet imply maturity.
The 28 included studies are dominated by preprints (82\%) and use at
least eight distinct LLMs as backbones.
No study in the corpus compares architectures on the same benchmark
with the same model, so the observed convergence reflects independent
author choices rather than an empirical validation of experience banks
over alternatives such as knowledge graphs or Git metaphors.
...
GBT~\citep{li2026traversalaspolicy} to roughly half the tokens of the
baseline for TALM~\citep{shen2025talm} on ClassEval,
...
Four are direct performance drops; the remaining five capture overhead,
maintenance burden, security, and evaluation artefacts.
Retrieval selection and backend governance are the dominant mechanisms
in the direct performance drops:
...
the figure rises to 12 of the 28 included studies (43\%) when
contextual sources are counted.
...
every analysed study, would make adoption decisions measurable rather
than accuracy-only.
...
\citet{wu2025gcc} reports a Git-metaphor context-management gain of
...
no study in the corpus compares experience banks against knowledge
graphs, hierarchical memory, or other architectures under controlled
conditions.
...
The three gaps confirmed by this review define the next empirical
step suggested by the evidence.
Gap~1, the absence of head-to-head comparisons, calls for an experiment
that evaluates at least three representative architectures on a shared
benchmark, with a fixed model, and under a matching compute budget.
Gap~2, cost underreporting, requires that such an experiment include
an integrated cost-per-task analysis, producing an explicit Pareto
