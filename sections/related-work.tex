...
memory management.
The authors' scope is explicitly broad, including agents for general
tasks such as web navigation, mathematical reasoning, and dialogue,
without restriction to the coding domain.
As a consequence, the specific coverage of software engineering in the
Du et al.\ survey is limited to ChatDev and MetaGPT, while systems
central to the present investigation, such as
MemCoder~\citep{deng2026memcoder},
CodeMEM~\citep{wang2026codemem}, and the Repository Memory approach of
\citet{wang2026improving}, are not mentioned.
...
\citet{wang2024llmagents} map the autonomous-agents landscape in
software engineering with a focus on execution pipelines, and
...
None of these reviews directly answers the secondary questions SQ1--SQ5
of this investigation: the taxonomy of persistence architectures, the
comparability of memory-specific benchmarks, the quantification of
gains over no-memory baselines, the cost-effectiveness relationship, or
the memory-induced degradation mechanisms.
...
dedicated dimension (SQ4), which constitutes the central differential
relative to prior work: none of the previously identified SLRs
simultaneously gathers a focus on cross-session persistence, a coding-
agent scope, and cross-study cost quantification.
This configuration enables answers to questions that earlier secondary
work does not address, chiefly the absence of head-to-head comparisons
between architectures, the systematic underreporting of costs, and the
