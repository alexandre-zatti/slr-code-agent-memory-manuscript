...
evaluation.
The remaining 24 studies propose or implement persistence architectures
and are analysed in the taxonomy;
...
Live-SWE-Agent~\citep{xia2025livesweagent} is retained as a task-local
boundary case whose executable tools are reused within a task but
discarded across tasks (\autoref{sec:selecao}).
...
\begin{figure}[!htbp]
  ...
  \caption{Distribution of the 24 architecture studies in the space of
    ...
    knowledge graphs, and Git metaphors remain at isolated
    adoption.}
  \label{fig:taxonomy}
\end{figure}
...
whose evaluation combines benchmark and in-the-wild deployment.
...
\begin{table}[htbp]
  ...
  \caption{Benchmarks and usage frequency across the 24 studies with
    ...
    benchmark;
    a single study may evaluate on more than one benchmark (so the
    column does not sum to 24).}
...
on the same benchmark with the same model, confirming Gap~1.
...
the remainder report cost under a single configuration or against a
different backbone rather than against an ablated memory baseline.
Adding the four contextual studies (the Du survey, for which the cost
field is not applicable, plus the Deshpande, Joshi, and Srivastava
studies, which do not report cost data) raises the count to 12 of the
28 included studies (43\%) without cost information.
The gap in cost reporting thus applies whether the denominator is the
architecture corpus (33\%) or the full included corpus (43\%), and
confirms Gap~2.

\autoref{tab:cost} reports monetary cost per task in the studies that
publish it; not every row in this table is a memory-vs-no-memory
contrast.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l r l l}
    ...
    Wang (RepoMem) & \$0.58--0.89 & NR & SWE-b. V. \\
    Zhang (AccelOpt) & \$6.87--15.95 & open-source & NKIBench \\
    Wang (MemGov., Claude) & \$7.27 & Claude 4 S. & SWE-b. V. \\
    \midrule
    \multicolumn{4}{l}{\footnotesize NR: not reported in the original paper.} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\citet{shen2025talm} consume roughly half the tokens of their
baseline on ClassEval.
...
\begin{figure}[!htbp]
  ...
  \caption{Schematic of the two opposite patterns of memory impact on
    operating cost reported among studies that provide a within-study
    cost or token delta against a no-persistence or ablation
    comparator.
    On the left, systems that substitute condensed memory for
    redundant context (e.g., EET, Traversal-as-Policy, TALM) report
    cost reductions.
    On the right, systems that add context via retrieval (e.g.,
    SWE-Exp, MemGovern with Claude~4~Sonnet) report cost increases.
    Each entry shows the variation reported in the original study
    relative to the study's own baseline or ablation comparator, using
    the variable the authors provide (percentage cost, percentage
    tokens, absolute token count, or a cost ratio);
    the figure is qualitative and the entries are not directly
    comparable across studies.
    CodeMEM's token reduction is attributed specifically to the Code
    Context Memory (CtxMem) component;
    removing the session-memory SessAST component in the same study
    reduces tokens further but at a performance cost, so the system's
    token profile depends on configuration.
    AccelOpt is omitted because its reported 26-fold cost reduction
    compares an open-source backbone with experience queue against a
    Claude~Sonnet~4 baseline rather than a within-study
    memory-versus-no-memory contrast.}
  \label{fig:cost}
\end{figure}
...
No study in the corpus constructs an explicit Pareto frontier between
accuracy and cost.
...
the studies that report cost data, so any apparent dominance reflects
comparison across heterogeneous settings rather than a controlled
evaluation on a shared frontier.
...
indicates which are direct performance drops and which are overhead,
security, or evaluation-artefact findings.

\begin{table}[htbp]
  \centering
  \caption{Adverse or cautionary findings related to persistence and
    its evaluation, grouped by mechanism category.
    Four cases document direct performance drops; the remainder
    capture overhead, security, or evaluation artefacts.}
  ...
  \begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.0cm} >{\raggedright\arraybackslash}p{2.4cm} >{\raggedright\arraybackslash}p{2.9cm} >{\raggedright\arraybackslash}X @{}}
    ...
    Lindenbauer & Performance drop & retrieval noise &
      Noisy items divert exploration ($-11$~pp) \\
    Chen & Performance drop & retrieval noise &
      retrieving $k > 1$ experiences degrades performance
      (42.0\% to 39.0\%) \\
    Deshpande & Performance drop & backend redundancy, follow-up decay &
    ...
    Xia & Performance drop & model-dependent overhead &
      Weak LLMs degrade $-68.2$\% under tool creation \\
    \midrule
    ...
    Vasilopoulos & Overhead & maintenance burden &
      Knowledge-to-code ratio of 24\% requires 1--2 h/week \\
    \midrule
...
Four studies report direct performance drops in their main or coded
adverse comparison:
...
Two report overhead or maintenance burden without a direct performance
drop~\citep{shen2025talm,vasilopoulos2026codified};
