...
\begin{figure}[!htbp]
  ...
  Each entry uses the variable reported by the original study
  (percentage cost, percentage tokens, absolute token count, or cost
  ratio).
  TALM is not plotted because its roughly 50\% token reduction is a
  full-system comparison against MapCoder; the memory module itself adds
  only marginal token overhead.
  CodeMEM's token reduction is attributed to the Code Context Memory
  (CtxMem) component;
  ...
  compares an open-source backbone with experience queue against a
  Claude~Sonnet~4 baseline rather than a memory-versus-no-memory
  contrast.
  \normalsize
\end{figure}
