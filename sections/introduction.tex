...
In CTIM-Rover, unfiltered memory items bias initial exploration;
in MemoryGraft, a small poisoned fraction appears disproportionately
in retrieved records under dual-channel
retrieval~\citep{lindenbauer2025ctimrover,srivastava2025memorygraft}.
...
Since 2024, 24 architecture studies relevant to persistence have
appeared in the corpus, including one task-local executable-memory
boundary case.
Architectures range from experience banks that distil resolution
trajectories to hierarchical state-based knowledge bases, AST-guided
memory, and repository-history or version-control memory.
These studies do not converge on a unified taxonomy, and evaluation
conditions remain heterogeneous across systems.

Three consequences follow for this review.
...
architecture studies (33\%) report no cost data, and 12 of the 28
included studies (43\%) do not contribute cost data when contextual
sources are counted, including one survey for which cost is not
applicable.
No study constructs a cross-architecture Pareto frontier under shared
...
More memory does not always produce more effectiveness.
...
SQ4 & Quantify coverage and cost-effectiveness &
  \autoref{sec:sq4} & \autoref{tab:cost-evidence}, \autoref{tab:cost},
  \autoref{fig:cost} \\
...
\begin{enumerate}
  ...
  \item empirical confirmation of three central gaps in the field:
    ...
    cost-accounting protocol, systematic underreporting of per-task cost, and
    insufficient coverage of the relationship between architectural
    complexity and effectiveness.
\end{enumerate}

The next empirical step should be a controlled comparative study,
as discussed in \autoref{sec:agenda-pesquisa}.
