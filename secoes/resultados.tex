...
\input{figuras/diagrama-fluxo-prisma}

\FloatBarrier
\subsection{Characteristics of the included studies}
\label{sec:caracteristicas}

\autoref{tab:caracteristicas} summarises the 28 included studies.
...
\autoref{fig:mapa-campo} offers a complementary chronological view of
...
\autoref{tab:arquiteturas} shows the distribution of the 24 primary
...
\autoref{fig:taxonomia} visualises the distribution of studies in the
temporal scope $\times$ substrate space, with colour indicating the
dominant control policy.

\begin{figure}[!htbp]
  \centering
  \resizebox{\textwidth}{!}{\input{figuras/fig-taxonomia}}
...
\autoref{tab:desempenho} lists the 17 whose gain is measurable in
...
LLM tested.
\autoref{fig:ganhos} visualises per-study gains, with colour indicating
architecture type.

\begin{figure}[!htbp]
  \centering
  \input{figuras/fig-ganhos}
...
\autoref{tab:custo} shows monetary cost per task in the studies that
publish it.
Variation is wide: from \$0.01 per
task~\citep{guo2026eet} to \$7.27~\citep{wang2026memgovern},
reflecting differences in LLM, benchmark, and architecture.
...
cost.
\autoref{fig:custo} illustrates this duality.

\begin{figure}[!htbp]
  \centering
  \resizebox{\textwidth}{!}{\input{figuras/fig-custo}}
...
performance.
\autoref{tab:degradacao} summarises these cases.
...
\autoref{tab:certeza} presents the per-finding certainty assessment.
...
For findings with moderate or low certainty (\autoref{tab:certeza}),
