...
(\autoref{tab:strings-busca}): agentes de IA para código (C1, 12 termos),
memória e persistência (C2, 13 termos) e domínio de engenharia de software
(C3, 8 termos).
Termos dentro de cada bloco são conectados por OR.

\begin{table}[htbp]
  \centering
  \caption{Termos de busca organizados por bloco conceitual.
           Combinação entre blocos: C1~AND~C2~AND~C3;
           dentro de cada bloco: OR.}
  ...
  \begin{tabularx}{\textwidth}{@{}
    ...
    Bloco & Termos \\
    \midrule
    C1 --- Agentes de IA para código (12)
    & \texttt{"LLM agent"},
...
No Scopus, os três blocos operam sobre título, resumo e palavras-chave
(\texttt{TITLE-ABS-KEY}) com filtro temporal
\texttt{PUBYEAR\,>\,2022}.
No arXiv, C1 e C2 operam sobre resumo (\texttt{abs:}) e, para quatro
termos de C1, também sobre título (\texttt{ti:}); o bloco C3 é
substituído pelo filtro de categorias \texttt{cs.SE}, \texttt{cs.AI} e
\texttt{cs.CL}, com filtro pós-busca \texttt{--year-from\,2023}.
As strings exatas por base, na sintaxe nativa de cada API, estão
disponíveis no pacote de replicação
...
Por tratar-se de síntese narrativa, não se aplicam medidas de efeito
formais (e.g., odds ratio, diferença padronizada de médias).
As medidas reportadas restringem-se a Pass@1 e variação absoluta em
pontos percentuais (\textit{delta-pp}).
