...
Nine of the 28 studies on knowledge persistence in coding agents report
scenarios in which memory hurts agent performance.
This finding makes it critical to distinguish when persisting knowledge
is beneficial and when it is harmful, a decision that precedes the
comparison among technically sophisticated architectures.
...
Systems such as SWE-Agent, OpenHands, and Agentless interact
iteratively with the source code, execute tests, and propose fixes
...
validated.
This knowledge volatility is equivalent to a developer without memory
between meetings.
...
The field, however, remains fragmented: studies do not converge on a
unified taxonomy, and most do not cite contemporary work on the same
problem.

From this fragmentation arise gaps that limit both research and
practice.
No study compares three or more persistence architectures on the same
benchmark with the same model; all evidence of architectural
superiority depends on comparisons across studies with heterogeneous
experimental conditions.
Cost underreporting compounds the problem: 43\% of the studies omit
cost data, and none constructs a Pareto frontier between accuracy and
computational cost.
...
presented in \autoref{sec:trabalhos-relacionados}.
...
Persistence adds overhead in storage, retrieval, and the agent's
effective context.
...
\begin{enumerate}
  \item the first systematic mapping of the field, classifying 24
    ...
    read/write policy;
  \item a quantitative synthesis of 19 controlled comparisons on
    effectiveness and cost, identifying diminishing returns on strong
    baselines and a bimodal impact on operating cost;
