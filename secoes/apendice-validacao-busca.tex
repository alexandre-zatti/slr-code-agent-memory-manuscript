...
A validação combina três procedimentos: auditoria das strings contra revisões sistemáticas comparáveis, verificação de recall por meio de um conjunto de artigos-semente operando como Quasi-Gold Standard (QGS), e registro do refinamento iterativo dos termos.
...
Após o refinamento documentado em \autoref{ap:refinamentos}, a recuperação alcançou seis dos sete artigos-semente; o caso restante (CodeMEM) foi resgatado pelo rastreamento de citações, conforme descrito na seção de método.
...
\subsection{Refinamentos iterativos}
\label{ap:refinamentos}

A estratégia passou por dois ciclos de refinamento documentados.
O primeiro ciclo, em 22 de março de 2026, removeu termos que produziam ruído alto (\textit{knowledge base} isolado) e substituiu termos pouco específicos (\textit{AI developer}, \textit{session}) por alternativas calibradas contra os artigos-semente que estavam falhando à recuperação.
O segundo ciclo, em 25 de março de 2026, decorreu da auditoria descrita em \autoref{ap:auditoria} e endereçou três fontes de erro adicionais: cobertura incompleta de variantes morfológicas, ausência de termos emergentes e assimetria entre as strings de Scopus e arXiv.
A \autoref{tab:refinamentos-busca} resume os refinamentos com a justificativa e o impacto medido.

\begin{table}[htbp]
\centering
\caption{Refinamentos iterativos das strings de busca, com impacto no recall.}
\label{tab:refinamentos-busca}
\small
\begin{tabular}{p{1.8cm}p{1cm}p{4.2cm}p{4.5cm}}
\toprule
\textbf{Data} & \textbf{Bloco} & \textbf{Mudança} & \textbf{Justificativa e impacto} \\
\midrule
2026-03-22 & C1 & Substituído \textit{AI developer} por \textit{language agent}. & Recupera Repository Memory; reduz ruído de papers sobre produtividade humana. \\
2026-03-22 & C2 & Substituído \textit{session} por \textit{memory-augmented}. & \textit{Session} demasiado genérico; nova forma captura agentes com memória. \\
2026-03-22 & C2 & Removido \textit{knowledge base} isolado. & Gerava ruído elevado em domínios não-SE. \\
2026-03-25 & C1 & Adicionado \textit{code assistant}. & Variante sem \textit{-ing} usada em três sistemas conhecidos. \\
2026-03-25 & C1 & Adicionado \textit{agentic coding}. & Termo emergente em venues 2025; +19 papers únicos no arXiv. \\
2026-03-25 & C1 & Adicionado \textit{SWE agent}. & Tornou-se nome de categoria; +12 papers únicos no arXiv. \\
2026-03-25 & C2 & Adicionados \textit{persistent memory}, \textit{persistent context}, \textit{cross-task}. & Frases recorrentes nos sistemas conhecidos; defendem recall no Scopus. \\
2026-03-25 & arXiv & Alinhamento de termos C1/C2 ausentes na string arXiv. & Corrigiu assimetria com Scopus; +53 papers. \\
2026-03-25 & arXiv & Adicionada busca por título (\texttt{ti:}) para termos C1 chave. & Recupera papers que mencionam \textit{coding agent} apenas no título; +13 papers e 2 sementes. \\
\bottomrule
\end{tabular}
\end{table}

A recuperação do conjunto de sementes evoluiu de quatro de sete antes do refinamento para seis de sete após o ciclo de 25 de março, e a contagem total de candidatos no arXiv passou de 876 para 973 (incremento de 11,1\%) sem queda perceptível na precisão estimada (mantida entre 40\% e 50\%).
O artigo-semente CodeMEM, único caso não recuperado por nenhuma das strings finais, é estruturalmente impossível de capturar pelo bloco C1 porque o resumo não contém qualquer termo da família \textit{agent} ou \textit{assistant}; sua inclusão final ocorre por rastreamento de citações forward a partir de Du~\cite{du2026memory}.
...
10 & Desenvolvimento da estratégia & Mapeamento de três blocos conceituais, construção das strings com OR/AND, validação por sete artigos-semente e dois ciclos de refinamento iterativo (\autoref{ap:refinamentos}). \\
