% Source: extraction/extracted-data.tsv (study, year, arch_family, seed_paper)
\begin{figure}[!htbp]
  \centering
  ...
  included studies, laid out by publication year (2024--2026) and
  architectural family or contextual role (colour). Double-bordered,
  bold-labelled markers identify the seven seed papers; square markers
  identify the three studies whose persistence substrate is executable
  (Live SWE-Agent, Darwin-G\"odel Machine, Traversal-as-Policy), with
  Live-SWE-Agent retained as a task-local boundary case.
  Families: experience banks (teal), hybrid (orange),
  repository-history or version-control memory (purple), AST-guided
  (pink), hierarchical/state knowledge base (green), text notes
  (yellow), contextual (grey).}
  \label{fig:field-map}
\end{figure}
