% Source: extraction/extracted-data.tsv (study, gain_pp, arch_family)
\begin{tikzpicture}[
  font=\small,
...
\def\entries{
  ...
  {Lindenbauer (CTIM-R.)/-2.0/archBanco}%
}
\foreach \name/\gain/\col [count=\i from 0] in \entries {
  \pgfmathsetmacro{\y}{-\i * 0.65}
  \pgfmathsetmacro{\w}{\gain * \scale}
  \node[bar label] at (-0.6, \y) {\name};
  \fill[\col!80] (0, \y - \barheight/2)
    rectangle (\w, \y + \barheight/2);
  \draw[\col!60!black, line width=0.3pt] (0, \y - \barheight/2)
    rectangle (\w, \y + \barheight/2);
  \pgfmathparse{\gain >= 0 ? 1 : 0}
  \ifnum\pgfmathresult=1
    \node[gain label] at (\w + 0.1, \y)
      {+\pgfmathprintnumber[fixed, precision=1]{\gain}};
  \else
    \pgfmathsetmacro{\absgain}{abs(\gain)}
    \node[anchor=east, font=\footnotesize] at (\w - 0.1, \y)
      {$-$\pgfmathprintnumber[fixed, precision=1]{\absgain}};
  \fi
}
\pgfmathsetmacro{\ybot}{-17 * 0.65 - 0.5}
\draw[thick, -Stealth] (0, 0.5) -- (0, \ybot);
\draw[thick] (-1.1, \ybot) -- (9.0, \ybot);
\foreach \v in {-5, 0, 5, 10, 15, 20, 25, 30, 35, 40} {
